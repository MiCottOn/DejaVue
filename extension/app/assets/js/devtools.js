...
chrome.devtools.panels.create('DejaVue', 'assets/img/logo.png', 'index.html', (
  ...
  chrome.storage.sync.get('oldCount', (data) => {
  ...
  show.then((_panelWindow) => {
    ...
    const stopTimeTravel = function () {
      hideButton();
      const evaluation2 =
				'inspect($$(".timeTravel"))[0].setAttribute("style", "width:100%; height: 100vh; background-color: #FFF; position: absolute; top: 0; left: 0; z-index: 99; display: none")';
      chrome.devtools.inspectedWindow.eval(evaluation2, () => {
    ...
    const timeTravelButton = document.createElement('button');
    ...
    _panelWindow.document.getElementById('treeContainer').appendChild(slider);
		...
		// append stop time travel button
    function showButton() {
      const treeviz = _panelWindow.document.getElementById('treeVisualization');
    ...
    function onRangeChange(rangeInputElmt, listener) {
      ...
      rangeInputElmt.addEventListener('input', (evt) => {
      ...
      rangeInputElmt.addEventListener('change', (evt) => {
    ...
    function onButtonClick(rangeInputElmt, listener) {
      rangeInputElmt.addEventListener('click', (evt) => {
        listener(evt.target.value);
      });
    }
    ...
    const timeTravel = function (index) {
      ...
      chrome.storage.local.get('states', (result) => {
        ...
        const newHTML = result.states[index][1].html;
        const stringHTML = JSON.stringify(newHTML);
        const evaluation = `inspect($$(".timeTravel"))[0].innerHTML=${stringHTML};`;
        chrome.devtools.inspectedWindow.eval(evaluation, () => {

        });
        const evaluation2 =
					`inspect($$(".timeTravel"))[0].setAttribute("style", "width:100%; height: 100vh; background-color: ${
					pageBgColor
  ...
  if (!rootNodes.includes(node) &&  node.__vue__)
    ...
    for (let i = 0; i < keysArray.length; i++) {
      if (!rootNodes.includes(node.children[keysArray[i]])) 
  ...
  while(components[0] === components[1]) 
    components.shift();
  ...
  //assigns name to node - if router-link attempts to get text content as name otherwise uses component tag as name - removes "vue-component-" and adds unique ID to end in case components have the same name (fixes parent/child relationships)
    if(node.$options._componentTag !== undefined) {
        if(node.$options._componentTag.includes("router-link")) {
          ...
          this.name = node.$options._componentTag + '-' + node._uid;
    }
  ...
  // $parent -> $options -> _componentTag
    ...
    if(node.$parent.$vnode === undefined) 
      this.parentName = 'Vuee';
    else if (node.$parent.$options._componentTag !== undefined) 
      this.parentName = node.$parent.$options._componentTag + '-' + node.$parent._uid;
    else {
  ...
  function createDvComps(components) {
    for (let i = 0; i < components.length; i += 1) {
      ...
      for (let j = 0; j < compElem.length; j++) {
          if (typeof compVals[j] === 'function') 
            dvComponents[dvComponents.length - 1].variables.push(compElem[j] + ': Function');
          else if (Array.isArray(compVals[j])) 
            dvComponents[dvComponents.length - 1].variables.push(compElem[j] + ': ' + JSON.stringify(compVals[j]));
          else if (typeof compVals[j] === 'string') 
            dvComponents[dvComponents.length - 1].variables.push(compElem[j] + ': "' + compVals[j].replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;") +'"');
          else 
            dvComponents[dvComponents.length - 1].variables.push(compElem[j] + ': ' + compVals[j]);
      }
  
      let methodKeys = Object.keys(node).filter((el) => {
        if(el[0] === '_' || el[0] === '$') 
          return false
        else if(typeof node[el] === 'function') 
          return true;
        else 
          return false;
      })
      ...
      if (methodKeys) {
        methodKeys.forEach((method) => {
          if (method) 
            dvComponents[dvComponents.length - 1].methods.push(method);
        });
      }
  ...
  const drawTree = (data, panel, dataIndex = data.length - 1) => {
    ...
    const slider = panel.document.getElementById('slider');
    ...
    if (dataCompare === dataIndex) return;
      ...
      if (data[dataIndex - 1]) {
        function compareDiff(data) {
          ...
          const compare = data[dataIndex - 1];

          for (let i = 0; i < current.length; i += 1) {
            for (let j = 0; j < compare.length; j += 1) {
              if (current[i].name === compare[j].name) {
                flag = JSON.stringify(current[i]) === JSON.stringify(compare[j]);
                if (!flag) {
                  const differences = DeepDiff(current[i], compare[j]);
									// remove positioning changes which are not state related
                  current[i].changes = differences.filter((el) => {
                    if (
											el.path.indexOf('top') > -1 ||
											el.path.indexOf('bottom') > -1 ||
											el.path.indexOf('left') > -1 ||
											el.path.indexOf('right') > -1
										)											{ return false; }
                    return true;
                  });
                }
  ...
  svg.attr('transform', d3.event.transform);
      ...
      function update(source) {
        ...
        let highlight;
        let removal;
        const changeDiv = panel.document.createElement('div');
        changeDiv.setAttribute(
					'style',
					'width:200px; height:200px; background-color: #FFF; color: #000',
				);
        ...
        nodeEnter
					...
					.attr('class', 'node')
					.attr('r', 1e-6)
					.style('fill', d => d._children ? '#42b983' : '#fff')
					.style('stroke', (d) => {
  ...
  chrome.devtools.inspectedWindow.eval(`highlight = document.createElement("div");
                            highlight.setAttribute('style', 'position: absolute; width: ${d
															.data.width}px; height: ${d.data.height}px; top: ${d.data.top}px; left: ${d.data.left}px; background-color: rgba(137, 196, 219, .6); border: 1px dashed rgb(137, 196, 219); z-index: 99999;')
  ...
  chrome.devtools.inspectedWindow.eval(`
					...
					.text(d => d.data.name.slice(0, d.data.name.lastIndexOf('-')));

				// color path to root for clicked node
        d3.selectAll('path').style('stroke', (d) => {
          if (d.color) {
  ...
  return diagonal(o, o);
        ...
        function clickHandler(d) {
          ...
          const select = d.data.name;

					// find selected data from flattened root record
          let find = flatten(root).find((d) => {
            if (d.data.name == select) return true;
          });

					// reset all the data to have color undefined.
          flatten(root).forEach((d) => {
            d.color = undefined;
          });
          ...
          function propToggle(e) {
            ...
            const element = panel.document.querySelector('.props-list').classList.toggle('opened');
          }

          function methodToggle(e) {
            e.preventDefault();
            panel.document.getElementById('methods_handler').classList.toggle('active-sidebar');
            const element = panel.document.querySelector('.methods-list').classList.toggle('opened');
          }
          ...
          const methodList = panel.document.getElementById(`${d.data.name}Methods`);
          ...
          const variableList = panel.document.getElementById(`${d.data.name}Variables`);
          ...
          const propList = panel.document.getElementById(`${d.data.name}Props`);
          ...
          if (d.data.props === 'undefined') {
            ...
            for (let i = 0; i < d.data.props.length; i += 1) {
              ...
              const propName = d.data.props[i][0];
              const propValue = d.data.props[i][1];
              const formattedProp = propMaker(propName, propValue);
                console.log(typeof d.data.props[i][1])
              propListItem.setAttribute('id', `${d.data.name}Prop`);
              propListItem.innerHTML = formattedProp;
          ...
          if (d.data.changes) {
            ...
            for (let i = 0; i < d.data.changes.length; i += 1) {
              const changes = d.data.changes[i];
              if (
								changes.path.includes('html') ||
								changes.path.includes('top') ||
								changes.path.includes('bottom') ||
								changes.path.includes('left') ||
								changes.path.includes('right')
							) {
                d.data.changes.splice(i, 1);
              } else {
                const prop = JSON.stringify(changes.path[0]).slice(1, changes.path[0].length + 1);
                const oldVal =
									typeof changes.rhs === 'number'
										? Math.floor(changes.rhs)
										: changes.rhs.slice(0, changes.rhs.length);
                const newVal =
									typeof changes.lhs === 'number'
										? Math.floor(changes.lhs)
										: changes.lhs.slice(0, changes.lhs.length);
                const changeText =
									prop === 'height' || prop === 'width'
										? `${prop} changed from ${oldVal} to ${newVal}`
										: `${oldVal} changed to ${newVal}`;
